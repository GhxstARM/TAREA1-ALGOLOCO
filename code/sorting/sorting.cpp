...
void saveArrayOutput(const vector<int>& arr, const string& algoritmo, const string& n_str, const string& t, const string& d, const string& m) {
    string output_array_filename =
        "data/array_output/" + n_str + "_" + t + "_" + d + "_" + m + "_out.txt";
...
void saveMeasurement(const string& algoritmo, int n, const string& t, const string& d, const string& m, double time_ms, long memory_kb) {
    ...
    if (!outfile.is_open()) {
        cerr << "No se pudo abrir el archivo CSV: " << csv_filename << "\n";
        return;
    }
...
int main(int argc, char* argv[]) {
    ...
    if (!infile.is_open()) {
        cerr << "no se pudo abrir el archivo de entrada: " << input_filename << "\n";
        return 1;
    }
    ...
    auto future_task = async(launch::async, [&]() {
        if (algoritmo == "quicksort") {
            ...
            cerr << "No se reconoce el algoritmo (" << algoritmo << ")\n";
            exit(1);
        }
    });

    
    if (future_task.wait_for(chrono::minutes(15)) == future_status::timeout) {
        cerr << "El algoritmo " << algoritmo << " supero el limite de 15 minutos con n=" << n << "\n";
        
        saveMeasurement(algoritmo, n, t, d, m, -1.0, -1);
        return 1; 
    }
    ...
    cout << "Prueba completada: " << algoritmo
        ...
        << " ms, Memoria adicional: " << memory_kb << " KB\n";
